...
# Import and merge
fields_to_copy =  ['monthly_IRREQ_1', 'monthly_IRREQ_2','monthly_IRREQ_3','monthly_IRREQ_4','monthly_IRREQ_5','monthly_IRREQ_6','monthly_IRREQ_7','monthly_IRREQ_8','monthly_IRREQ_9','monthly_IRREQ_10','monthly_IRREQ_11','monthly_IRREQ_12','IRREQ_year',]
...
clusters["q"] = clusters.loc[:, [x for x in clusters.columns if x.startswith('monthly')][0:12]].max(axis=1)/30/nhours_irr/3600
...
clusters["warning"] = numpy.where((clusters["q"]<clusters["qc1"]) & (clusters["q"]<clusters["qc2"]), 0, 1)

# Sustainable groundwater pumping rate
clusters["q_sust"] = numpy.where((clusters["q"]<clusters["qc1"]) & (clusters["q"]<clusters["qc2"]), clusters["q"], clusters[['qc1','qc2']].min(axis=1))

# Unmet demand due to unsustainable pumping
clusters["q_diff"] = clusters["q"] - clusters["q_sust"]

# RGH to estimate power for pump (in W), missing the head losses
clusters['powerforpump'] = (rho* g * clusters['gr_wat_depth_mean']* clusters["q_sust"])/eta_pump

clusters['powerforpump'] = numpy.where(clusters["gr_wat_depth_mean"]>15, 0, clusters['powerforpump'])
...
clusters["surfw_q"] = numpy.where(clusters["nogroundwater"] == 0, clusters["q_diff"], clusters["q"])

# But put a distance limit 
clusters["surfw_q"] = numpy.where(clusters["surfw_dist_mean"]<=threshold_surfacewater_distance, clusters["surfw_q"], 0)
...
clusters["imposs_wat"] = numpy.where((clusters["q_diff"]!=0) & (clusters["surfw_dist_mean"]>threshold_surfacewater_distance), 1, 0)
...
clusters["surfw_w"] = clusters["surfw_q"]*((32 * water_speed * clusters["surfw_dist_mean"] *water_viscosity)/pipe_diameter**2)/eta_pump

#Calculate monthly electric requirement
clusters['wh_monthly'] = clusters['powerforpump']*nhours_irr*30 + clusters['surfw_w']*nhours_irr*30
clusters['er_kwh'] = clusters['wh_monthly']*12/1000
...
processing.run("native:joinattributestable", {
        ...
        'FIELD_2': 'id', 'FIELDS_TO_COPY': ["q", "qc1",	"qc2", "warning", "q_sust", "q_diff", "powerforpump", "surfw_q", "imposs_wat",	"surfw_w",	"wh_monthly",	"er_kwh"
...
#https://www.cgdev.org/sites/default/files/if-you-build-it-will-they-consume-key-challenges-universal-reliable-and-low-cost.pdf
clusters['PerHHD'] = numpy.where(clusters['isurbanmajority'] >=12, clusters['acc_pop_share_t1_new'] * urb1 + clusters['acc_pop_share_t2_new'] * urb2 +  clusters['acc_pop_share_t3_new'] * urb3 +  clusters['acc_pop_share_t4_new'] * urb4, numpy.where(clusters['isurbanmajority'] <= 12, clusters['acc_pop_share_t1_new']* rur1 + clusters['acc_pop_share_t2_new'] * rur2 +  clusters['acc_pop_share_t3_new'] * rur3 +  clusters['acc_pop_share_t4_new'] * rur4, 0))
...
processing.run("native:joinattributestable", {
        ...
        'FIELD_2': 'id', 'FIELDS_TO_COPY': ['acc_pop_share_t1_new', 'acc_pop_share_t2_new', 'acc_pop_share_t3_new', 'acc_pop_share_t4_new', 'PerHHD', 'HHs'], 'METHOD': 1, 'DISCARD_NONMATCHING': False, 'PREFIX': '',
...
clusters['kwh_proc_cr'] = clusters[[col for col in clusters if col.startswith('kwh')]].sum(axis=1)

clusters.to_csv(processed_folder + 'clusters_7.csv')
clusters = QgsVectorLayer(processed_folder + 'clusters_16.gpkg',"","ogr")

processing.run("native:joinattributestable", {
        'INPUT': processed_folder + 'clusters_16.gpkg', 'FIELD': 'id',
        'INPUT_2': processed_folder + 'clusters_7.csv',
        'FIELD_2': 'id', 'FIELDS_TO_COPY': ['kwh_proc_cr'], 'METHOD': 1, 'DISCARD_NONMATCHING': False, 'PREFIX': '',
        'OUTPUT': processed_folder + 'clusters_17.gpkg'})
...
# health['heal_type2'] = (health['Type']=='Medical Clinic') | (health['Type'] == 'Dental Clinic') | (health['Type'] == 'Eye Centre') | \
 # (health['Type'] == 'Laboratory (Stand-alone)') | (health['Type'] == 'Medical Centre') | (health['Type'] == 'Radiology Unit') | \
 # (health['Type'] == 'Regional Blood Tranclustersusion Centre')
# health['heal_type3'] = (health['Type']=='Maternity Home') | (health['Type']=='Nursing Home') | (health['Type']=='VCT Centre')
# health['heal_type4'] = (health['Type']=='Health Centre') | (health['Type']=='Health Programme') | (health['Type']=='Health Project') | \
 # (health['Type']=='Rural Health Training Centre') | (health['Type']=='Training Institution in Health (Stand-alone)')
# health['heal_type5'] = (health['Type']=='District Hospital') | (health['Type']=='National Referral Hospital') | (health['Type']=='Other Hospital') | \
 # (health['Type']=='Provincial General Hospital') | (health['Type']=='Sub-District Hospital')
...
# ONLY DIFFERENCE BETWEEN SCHOOLS IS PUBLIC vs PRIVATE SCHOOLS
...
clusters = QgsVectorLayer(processed_folder + 'clusters_17.gpkg',"","ogr")
files = ['pupils_1.tif', 'pupils_2.tif', 'pupils_3.tif', 'pupils_4.tif', 'pupils_5.tif', 'sch_type1.tif', 'sch_type2.tif', 'sch_type3.tif', 'sch_type4.tif', 'sch_type5.tif']
...
clusters['beds_1.tifsum'] = clusters['tier1']
clusters['beds_2.tifsum'] = clusters['tier2']
clusters['beds_3.tifsum'] = clusters['tier34']*0.6
clusters['beds_4.tifsum'] = clusters['tier34']*0.3
clusters['beds_5.tifsum'] = clusters['tier34']*0.1

clusters['el_dem_hc'] = clusters['beds_1.tifsum'] * health1  + clusters['beds_2.tifsum'] * health2 + clusters['beds_3.tifsum'] * health3 + clusters['beds_4.tifsum'] * health4 + clusters['beds_5.tifsum'] * health5

# muliply by beds and divide by 20 or other values (Nick)
#clusters['beds_1.tifsum'] = clusters['tier1']
#clusters['beds_2.tifsum'] = clusters['tier2']*45
#clusters['beds_3.tifsum'] = clusters['tier34']*0.6*150
#clusters['beds_4.tifsum'] = clusters['tier34']*0.3*450
#clusters['beds_5.tifsum'] = clusters['tier34']*0.1*2000
#
#clusters['el_dem_hc'] = clusters['beds_1.tifsum'] * health1  + clusters['beds_2.tifsum'] * health2/45 + clusters['beds_3.tifsum'] * health3/150 + clusters['beds_4.tifsum'] * health4/450 + clusters['beds_5.tifsum'] * health5/2000

clusters['el_dem_sch'] = clusters['pupils_1.tifsum'] * edu1/500 + clusters['pupils_2.tifsum'] * edu2/500 + clusters['pupils_3.tifsum'] * edu3/500 + clusters['pupils_4.tifsum'] * edu4/500 + clusters['pupils_5.tifsum'] * edu5/500
...
#clusters['el_dem_hc'] = numpy.where(clusters['elrate'] > 0.75, 0,  clusters['el_dem_hc'])
#clusters['el_dem_sch'] = numpy.where(clusters['elrate'] > 0.75, 0, clusters['el_dem_sch'])

# Write to csv file
clusters.to_csv(processed_folder + 'clusters_14.csv')

#Write to file
processing.run("native:joinattributestable", {
        'INPUT': processed_folder + 'clusters_19.gpkg', 'FIELD': 'id',
        'INPUT_2': processed_folder + 'clusters_14.csv',
        'FIELD_2': 'id', 'FIELDS_TO_COPY': ['el_dem_hc', 'el_dem_sch'], 'METHOD': 1, 'DISCARD_NONMATCHING': False, 'PREFIX': '',
        'OUTPUT': home_repo_folder + 'clusters_final.gpkg'})
