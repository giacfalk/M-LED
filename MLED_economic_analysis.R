...
results_csv = read.csv(paste0(home_repo_folder, 'onsset/results/ke-1-0_1_0_0_1_0.csv'))

clusters <- merge(clusters, results_csv, by="id")

###
# Translation to be operated from here
##
...
for (X in files){
  ...
  b <- raster(paste0(spam_folder, "spam2010v1r0_global_yield.geotiff/", X))
  values(b) <- ifelse(b==0, NA, b)

  clusters[a] <- exactextractr::exact_extract(b, clusters, fun="mean")
...
files = list.files(path = paste0(spam_folder, "spam2010v1r0_global_yield.geotiff") , pattern = 'i.tif')

list_i <- list()

for (X in files){
  a = paste0("Y_" , gsub("_i.tif", "", gsub("spam2010v1r0_global_yield_", "", X)))
  list_i[[X]] <- as.data.frame(zonal(a, climatezones, fun='mean', digits=2, na.rm=TRUE))
